...
int main(void) {
    ...
    if (pthread_create(&prod_thread1, NULL, produtor, NULL) != 0) {
        perror("Erro ao criar thread produtora");
        exit(1);
    }

    if (pthread_create(&prod_thread2, NULL, produtor, NULL) != 0) {
        perror("Erro ao criar thread produtora");
        exit(1);
    }

    if (pthread_create(&cons_thread1, NULL, consumidor, NULL) != 0) {
        perror("Erro ao criar thread consumidora");
        exit(1);
    }

    if (pthread_create(&cons_thread2, NULL, consumidor, NULL) != 0) {
        perror("Erro ao criar thread consumidora");
        exit(1);
    }

    // Espera as quatro threads terminarem
